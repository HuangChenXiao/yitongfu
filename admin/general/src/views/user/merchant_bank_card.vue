  ...
  <div class="app-container calendar-list-container">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" :close-on-click-modal="false">
      <el-form class="small-space" :model="ruleForm" :rules="rules" ref="ruleForm" label-position="left" label-width="150px" style='width: 620px; margin-left:50px;'>
        <el-form-item label="商户名称" prop="businessid">
          <el-select v-model="ruleForm.businessid" filterable placeholder="请选择商户编号" style="width:100%">
            <el-option
...
export default {
  data() {
    ...
    return {
      ...
      rules: {
        businessid: [
          { validator:businessid_change, trigger: "change" }
        ],
        mobileNo:[{ required: true, message: "请输入持卡人手机号", trigger: "blur" }],
        handleType:[{ required: true, message: "请选择操作类型", trigger: "change" }],
        bankCode:[{ required: true, message: "请选择银行代码", trigger: "change" }],
        bankaccProp:[{ required: true, message: "请选择账户属性", trigger: "change" }],
        name:[{ required: true, message: "请输入持卡人姓名", trigger: "blur" }],
        bankaccountType:[{ required: true, message: "请选择银行卡类型", trigger: "change" }],
        certCode:[{ required: true, message: "请选择办卡证件类型", trigger: "change" }],
        certNo:[{ required: true, message: "请输入证件号码", trigger: "blur" }],
        bankbranchNo:[{ required: true, message: "请输入联行号", trigger: "blur" }],
        defaultAcc:[{ required: true, message: "请选择默认账户", trigger: "change" }],
        province:[{ required: true, message: "请输入开户省份", trigger: "blur" }],
        city:[{ required: true, message: "请输入开户城市", trigger: "blur" }],
        bankaddress:[{ required: true, message: "请输入开户支行", trigger: "blur" }],
      }
    };
  },
  computed: {},
  ...
  methods: {
        ...
        merchBankCardRegister("get", {
            ...
            if (response.data.returnState) {
              ...
              if (resdata.status_code == 1) {
                ...
                editmerchantbankcard("put", this.ruleForm).then(response => {});
                this.$notify({
                    title: "成功",
                    message: "审核成功",
                    type: "success",
    ...
    fetchData() {
      ...
      merchantbankcard("get", this.listQuery).then(response => {
    ...
    getbasic() {
      ...
      businessbasic("get").then(response => {
    ...
    create(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          editmerchantbankcard("post", this.ruleForm).then(response => {
              this.dialogFormVisible = false;
              this.fetchData();
              this.$notify({
                title: "成功",
