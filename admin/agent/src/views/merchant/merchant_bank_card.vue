      ...
      <el-form class="small-space" :model="ruleForm" :rules="rules" ref="ruleForm" label-position="left" label-width="150px" style='width: 620px; margin-left:50px;'>
        <el-form-item label="商户名称" prop="businessid">
          <el-select v-model="ruleForm.businessid" filterable placeholder="请选择商户编号" style="width:100%">
            <el-option v-for="item in business_basic" :key="item.id" :label="item.shortName" :value="item.id">
            </el-option>
          </el-select>
        </el-form-item>
...
export default {
  data() {
    ...
    return {
      ...
      rules: {
        businessid: [{ validator: businessid_change, trigger: 'change' }],
        mobileNo: [
  ...
  methods: {
    ...
    getbasic() {
      ...
      FaBusinessBasic('get').then(response => {
