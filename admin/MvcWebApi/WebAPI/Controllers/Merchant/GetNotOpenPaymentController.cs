...
{
    public class GetNotOpenPaymentController : ApiController
    {
        ...
        public ResponseMessageResult GetOpenPayment()
        {
            ...
            if (jwtmodel.isadmin)
            {
                ...
                var query = db.Query("select * from [dbo].[fa_business_basic] a where  not exists(select 1 from fa_open_payment b where a.appid=b.merchantId) and a.isaudit=1 order by id desc").ToList();
